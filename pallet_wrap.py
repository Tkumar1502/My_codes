...
#define scenario
processing_capacity=40e3 #tons / year feedstock

#single wash
process = strap.BaselineSTRAPProcess(
    scenario='PE/Xylene',
    target_plastic_percent=95, # %
...
process.tea.labor_cost = 1600000 # (2 operators x $60K, 1 loader x $50k (2 loaders for large scale), 1 engineer X $100k)x5
...
process.PE_resin.price = 0.95#$1.20 / kg
process.leftover_plastic.price = -0.07 # $8.00 / kg --> set to zero if not selling
...
process.set_feedstock_price.bounds=(0.03,0.46)
...
#set a capacity, get back sales-AOC to be used to find minimum capacity of break even
def sales_minus_aoc(capacity):
...
#plot feedstock price and sale price to get a range of capacities of plants
def capacity_contour(feed_lb, feed_ub, sale_lb, sale_ub):
    # Define x and y bounds
    sale_prices = np.linspace(sale_lb, sale_ub, 10)
    feedstock_prices = np.linspace(feed_lb, feed_ub, 10)

    # Create a 2D array to hold the capacity values
    caps= np.zeros((len(feedstock_prices), len(sale_prices)))

    for i, cost in enumerate(feedstock_prices): 
        for j, price in enumerate(sale_prices):
            process.feedstock.price = cost
            process.PE_resin.price = price
            cap = fsolve(sales_minus_aoc, 9000)
            caps[i, j] = cap[0]
            
    print(caps)

    # Create meshgrid
    X, Y = np.meshgrid(feedstock_prices, sale_prices)
    ...
    contourf = ax.contourf(X, Y, caps, levels=levels, cmap='RdYlGn', norm=norm)

    # Add contour lines and labels (only for positive levels)
    contour_lines = ax.contour(X, Y, caps, levels=pos_levels, colors='black', linewidths=0.5)
    ...
    # Labels and title
    ax.set_xlabel('Feedstock cost ($/kg)')
    ax.set_ylabel('STRAP resin sale price ($/kg)')
    ax.set_title('Break-even capacity')
...
def droi_plot():
    ...
    # Save data to Excel
    output_folder = './results/pallet_wrap/'
...
def msp_confidence():
    TCIs = [TCI(i)/1000000 for i in pcs]
   ...
   # Add legend manually for Monte Carlo results
    ...
    # Add other plots
    #line2, = plt.plot(pcs, market, 'r--', label='Market Value PE')  # Market value
    plt.axhspan(.80, 1.20, color='blue', alpha=0.15, label='Market Value Range')
    plt.ylabel('MSP of STRAP resin ($/kg)')
    plt.xlabel('Feedstock Processing Capacity (metric ton/year)')
...
def save_lca_tables_to_excel():
    ...
    filename="./results/pallet_wrap/LCA_results.xlsx"
...
def msp_tornado():
    ...
    bst.plots.plot_single_point_sensitivity(baseline[metric_index],
                                            lower[metric_index],
                                            upper[metric_index],
                                            name='Minimum selling price ($/kg product)',
                                            w=1.0,
                                            index=index)
...
def save_system_reports():
    folder = './results/super_sacks/'
    #folder = os.path.join(folder, 'results')
    for scenario in ('experimental', 'NREL'):
        pm = process
        filename = f'supersacks_40kton_{scenario}_detailed_report.xlsx'
        file = os.path.join(folder, filename)
        pm.system.save_report(file)
        
def save_detailed_expenditure_tables(sigfigs=3):
    folder = './results/super_sacks/'   #set this to your own file directory
    #folder = os.path.join(folder, 'results')
    filename = '40kton_plant_expenditures.xlsx'
    file = os.path.join(folder, filename)
    writer = pd.ExcelWriter(file)
    product = 'PE_resin', 'leftover_plastic'
    process_models = [
        process
    ]
