...
# -*- coding: utf-8 -*-
"""
Created on Tue May 13 09:36:53 2025
...
#bounds 1-wash
process.set_polymer_mass_fraction.bounds=(.85, .95)
process.set_dissolution_capacity.bounds=(2.5, 10)
...
process.set_feedstock_price.bounds=(0.0,0.30)
...
process.set_polymer_mass_fraction.baseline = 0.92
process.set_dissolution_capacity.baseline = 5
process.set_precipitation_temperature.baseline = 45+273
process.set_feedstock_price.baseline=(0.10)
...
def toc_kg(pc):
    ...
    process.system.simulate()
    opex = process.tea.AOC
    kg = pc * process.target_plastic_percent * 1000
    return opex / kg
...
#plot operating cost/kg vs scale
def opex_kg_plot():
    costs = [toc_kg(i) for i in pcs]
    TCIs = [TCI(i) for i in pcs]
    
   # Create a DataFrame with PCS, MSPs, and TCIs
    df = pd.DataFrame({'pcs': pcs, 'Cost': costs, 'TCI': TCIs})
    
   #plot data
    plt.figure()
    line1, = plt.plot(pcs, costs, label='STRAP OPEX/kg') # plot MSP for different capacities
    #line2, = plt.plot(pcs, market, 'r--', label='Market Value PE') #market value
    plt.ylabel('oeprating cost to produce STRAP resin ($/kg)')
    plt.xlabel('Plant Capacity (mton/year)')
...
def droi_plot():
    ...
    # Save data to Excel
    output_folder = './results/super_sacks/'
...
def save_lca_tables_to_excel():
    ...
    filename="./results/super_sacks/LCA_results.xlsx"
...
def save_system_reports():
    ...
    for scenario in ('experimental'):
        pm = process
        filename = f'TESTsuper_sacks_{scenario}_detailed_report.xlsx'
