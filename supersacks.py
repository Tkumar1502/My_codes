...
# -*- coding: utf-8 -*-
"""
Created on Tue May 27 12:25:20 2025
...
# why is there no PE lost into the waste dryer?
# how do we account for extra emissions from burning the plastic?
# how do we add extra CFs like toxicity?
...
#define scenario
processing_capacity=36e3 #tons / year feedstock

#single wash
process = strap.BaselineSTRAPProcess(
    scenario='PE/Xylene',
    target_plastic_percent=0.94, # %
    processing_capacity=processing_capacity, # MT/yr
    sell_leftover_plastic=False, #undissolved fraction
...
process.leftover_plastic.price = 0.00 # $8.00 / kg --> set to zero if not selling


#bounds 1-wash
process.set_polymer_mass_fraction.bounds=(.90, .96)
...
process.set_feedstock_price.bounds=(0.03,0.08)
...
def cap_by_price(lb, ub, low_price, high_price):
    ...
    dROI = np.zeros((len(prices), len(capacities)))
    ...
    for i, cap in enumerate(capacities): 
        for j, price in enumerate(prices):
            ...
            process.system.simulate()
            dROI[i,j] =  process.tea.NPV / process.tea.TCI
    
            
    # Create the normalization: center at 0
    norm = TwoSlopeNorm(vmin=np.min(dROI), vcenter=0, vmax=np.max(dROI))
...
def save_system_reports():
    ...
    for scenario in ('experimental', 'NREL'):
        pm = process
        filename = f'TESTpallet_wrap_{scenario}_detailed_report.xlsx'
        file = os.path.join(folder, filename)
        pm.system.save_report(file)
        
def save_detailed_expenditure_tables(sigfigs=3):
    folder = './results/pallet_wrap/'   #set this to your own file directory
    #folder = os.path.join(folder, 'results')
