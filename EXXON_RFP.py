...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun 24 09:42:17 2025
...
process.leftover_plastic.price = 0#8.00 # $8.00 / kg
...
process.set_polymer_mass_fraction.baseline = 0.90
...
def slice_irr():
    ...
    # Save results
    np.savez_compressed("./results/exxon/rr_grid.npz",
    ...
    df.to_excel("./results/exxon/irr_grid_data.xlsx", index=False)
    ...
    plt.savefig('./results/exxon/irr_contour_stack.pdf', bbox_inches='tight') 
    plt.show()
...
def irr_contour(lb, ub, low_price, high_price):
    ...
    plt.savefig('./results/exxon/irr_contour.pdf', bbox_inches='tight') 
...
def save_lca_tables_to_excel():
    ...
    filename = "./results/exxon/LCA_results.xlsx"
...
def save_system_reports():
    folder = './results/exxon'
    #folder = os.path.join(folder, 'results')
    for scenario in ('experimental', 'NREL'):
        pm = process
        filename = f'TESTexxon_{scenario}_detailed_report.xlsx'
